...
let recent=store.get('ph_recent_v4',[]), lastQuery=store.get('ph_last_query_v4',store.get('ph_last_query',''));
...
function runSearch(){const raw=$('#searchInput').value.trim();if(!raw)return;if(!context){alert('Choose a vehicle first.');return}lastQuery=raw;store.set('ph_last_query_v4',raw);recent=[raw,...recent.filter(x=>x.toLowerCase()!==raw.toLowerCase())].slice(0,8);store.set('ph_recent_v4',recent);renderRecent();renderSources(raw);renderResults(matches(raw))}
...
async function importData(f){try{const d=JSON.parse(await f.text());if(d.app!=='PartHound')throw Error();vehicles=d.vehicles||vehicles;context=d.context||context;saved=d.saved||saved;listings=d.listings||listings;recent=d.recent||recent;lastQuery=d.lastQuery||'';serviceNotes=d.serviceNotes||serviceNotes;torqueSpecs=d.torqueSpecs||torqueSpecs;store.set('ph_service_notes_v43',serviceNotes);store.set('ph_torque_specs_v44',torqueSpecs);$('#projectNotes').value=d.notes||'';store.set('ph_vehicles_v4',vehicles);store.set('ph_search_vehicle_v4',context);store.set('ph_saved_v4',saved);store.set('ph_listings_v4',listings);store.set('ph_recent_v4',recent);localStorage.setItem('ph_notes',d.notes||'');initRender();alert('Backup imported.')}catch{alert('That is not a valid PartHound backup.')}}
